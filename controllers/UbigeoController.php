...
class UbigeoController extends Controller
{
    ...
    {         
        ...
        $instituciones=Institucion::find()
                    ->select('id,denominacion,codigo_modular,ugel')
                    ->where('ubigeo_id=:ubigeo_id and estado=1',[':ubigeo_id'=>$distrito])
                    ->groupBy('id,denominacion,codigo_modular')
                    ->orderBy('denominacion asc')->all();
        
        $institucionesGroups=Institucion::find()
                    ->select('ugel')
                    ->where('ubigeo_id=:ubigeo_id and estado=1',[':ubigeo_id'=>$distrito])
                    ->groupBy('ugel')
                    ->orderBy('ugel asc')->all();
                    
        if($countInstitucion>0){
            echo "<option value></option>";
            foreach($institucionesGroups as $institucionesGroup)
            {
                echo "<optgroup label='$institucionesGroup->ugel'>";
                foreach($instituciones as $institucion){
                    if($institucion->ugel==$institucionesGroup->ugel)
                    {
